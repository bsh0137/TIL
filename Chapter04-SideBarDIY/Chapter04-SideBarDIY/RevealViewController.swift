...
class RevealViewController: UIViewController {
    ...
    func setupView() {
        ...
        if let vc = self.storyboard?.instantiateViewController(identifier: "sw_front") as? UINavigationController {
            ...
            // 4) 프론트 컨트롤러의 델리게이터 변수에 참조 정보를 넣어준다.
    ...
    func openSideBar(_ complete: ( () -> Void)?) {
        // 1) 앞에서 정의했던 메소드들을 실행
        self.getSideView() // 사이드 뷰 바를 읽어온다.
        ...
        UIView.animate(withDuration: TimeInterval(self.SLIDE_TIME), delay: TimeInterval(0), options: options, animations: {
            ...
            if $0 == true {
                self.isSideBarShowing = true // 열린 상태로 플래그를 변경한다.
                complete?()
            }
        })
    ...
    func closeSideBar(_ complete: ( ()-> Void)?) {
        ...
        // 애니메이션 실행
        UIView.animate(withDuration: SLIDE_TIME, delay: TimeInterval(0), options: options, animations: {
