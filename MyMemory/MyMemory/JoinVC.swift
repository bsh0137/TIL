...
class JoinVC: UIViewController, UITableViewDataSource, UITableViewDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate {
    ...
    // 테이블 뷰에 들어갈 텍스트 필드들
    var fieldAccoount: UITextField! // 계정 필드
    ...
    @IBAction func submit(_ sender: Any) {
        ...
        let param: Parameters = [
            "account"       :self.fieldAccoount.text!,
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 0:
            self.fieldAccoount = UITextField(frame: tfFrame)
            self.fieldAccoount.placeholder = "계정(이메일)"
            self.fieldAccoount.borderStyle = .none
            self.fieldAccoount.autocapitalizationType = .none
            self.fieldAccoount.font = UIFont.systemFont(ofSize: 14)
            cell.addSubview(self.fieldAccoount)
        case 1:
